...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/borrowing-request/:documentId/download', requireAuth, async (req, res) => {
    try {
      ...
      doc.text(`Document ID: ${document.documentId}`, margin, doc.y, { width: contentWidth, align: 'center' });
      doc.text(`Generated by BONEVET Inventory Management System`, margin, doc.y, { width: contentWidth, align: 'center' });
      doc.text(`Date: ${new Date().toLocaleDateString()}`, margin, doc.y, { width: contentWidth, align: 'center' });
